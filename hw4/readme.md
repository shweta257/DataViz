...
If you're not planning on doing the extra credit hacker version, you can move on to Part II. We suggest, however, that you take a look at the JSON file as it describes the data structure you will use. 

## Part I: Data Aggregation (HACKER ONLY)

Your first task is to set up the table in the `createTable()` function. Notice the absence of input parameters as you can access the necessary data from the global variable created when you read in the data from ``fifa-matches.csv``.  We have already provided a table element in hw4.html, with an appropriately named id of `matchTable` and with the column headers set up.
  
### Aggregating Match Information
...
Recall from earlier how our table will be composed of two types of rows. Aggregate rows that contain summary values for all games, and game specific rows that contain information for a given match. In order to facilitate the process of populating this table, we will be keeping an updated list of all data elements we want to display in the table in the global variable `tableElements`. 

So far we have only explicitly created aggregate data objects so at this point you should populate `tableElements` with the existing array of aggregate data objects.  
...
The next step is to implement the `updateTable()` function, which will populate the table, based on the data stored in `tableElements`. Later we will implement the ability to add, delete, or re-sort rows based on user interaction. 
...
5%: Part I: Data is loaded in properly. Only one call to load is made for each file.   
10%: Part II: Table setup is done properly, particularly the data aggregation step.  
30%: Part III: Table is properly populated.   
...
15%: Part VI: Tree is properly rendered.  
...
10%: Extra Credit: Implement the Hacker Version  
